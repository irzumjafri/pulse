package com.solita.pulse.ui // Adjust package name if needed
...
// Import your project's BuildConfig generated by Gradle
...
import com.solita.pulse.speech.HotwordDetector // Import HotwordDetector
...
import android.media.ToneGenerator // <<< Import ToneGenerator
...
class AssistantViewModel(application: Application) : AndroidViewModel(application), SpeechEventListener {
    ...
    // --- Tone Constants ---
    private val START_LISTENING_TONE = ToneGenerator.TONE_PROP_ACK // A short confirmation tone
    private val STOP_LISTENING_TONE = ToneGenerator.TONE_PROP_BEEP // A standard beep
    private val TONE_DURATION_MS = 150 // Duration in milliseconds
    ...
    private fun initializeHotwordDetector() {
        ...
        if (accessKey == "YOUR_ACCESS_KEY_HERE" || accessKey.isBlank() || !accessKey.matches(Regex("^[a-zA-Z0-9/+=]+$"))) {
            Log.e("AssistantViewModel", "Picovoice Access Key missing/invalid. Hotword detection disabled.")
    ...
    internal fun processHotword(isChatMode: Boolean) {
    ...
    fun setLocale(newLocale: Locale) {
        ...
        currentNetworkJob = viewModelScope.launch {
            val result = NetworkUtils.resetUserContext(sessionID)
            val errorUpdate = when (result) {
    ...
    override fun onRecognitionStarted() { Log.d("SpeechManager Event", "onRecognitionStarted")
        playTone(START_LISTENING_TONE, TONE_DURATION_MS)}
...
//            lastInteractionWasHotword = false // Reset flag
    ...
    override fun onRecognitionStopped() {
        ...
        playTone(STOP_LISTENING_TONE, TONE_DURATION_MS)
    ...
    private fun handleCancellationOrError(message: String, requestId: String?, isCancellation: Boolean) {
        ...
        _uiState.update { state ->
            ...
            val updatedHistory = if (lastIndex >= 0 && state.chatHistory[lastIndex].second == MessageType.Server && state.chatHistory[lastIndex].first.contains("Thinking")) {
                ...
                state.chatHistory.dropLast(1) + (finalMessage to MessageType.Server)
